...
/* Développement/Production Revproxy Functions */

function get_revproxy_info($table) {
  include('database/database_conn_revproxy.php');
  $query = "SELECT * FROM data WHERE categorie='".$table."' 
...
function display_data_revproxy($arr) {
  foreach($arr as $item) {
    $url = "https://".$item["nom_domaine"];
    $httpcode = http_code($url);
    echo '
    <div class="col-3 col-4-medium col-12-small">
      <section class="box style1">
        <span class="icon featured '.icon_up_down($httpcode).'"></span>
        <h3>'.preg_split('/\./', $item["nom_domaine"], -1, PREG_SPLIT_NO_EMPTY)[0].'</h3>
        <p></p>
        <ul>
          <li>HTTP Code: '.$httpcode.'</li>
          <li><a href="'.$url.'">Accéder au site</a></li>
